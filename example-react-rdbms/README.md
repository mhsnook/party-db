...
1. **A real structured table.** The server owns a `todos` table with typed columns
   and CRUDs into it (structured writes), instead of the schemaless `(key, data)`
   blob the other examples use.
2. **Auth.** Reads are open to everyone, but **writing needs a password**
...
## What's different from the [schemaless example](../example-react)

Everything else is identical; only these lines change.
...
export default {
  fetch: (req, env) =>
    ...
    routePartykitRequest(req, env, authHooks(authorize)).then((r) => r ?? new Response('not found', { status: 404 })),
}
```
