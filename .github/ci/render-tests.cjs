...
//
// `parse` reads the JSON shape Vitest writes with `--reporter=json`, which is
// the Jest shape: `testResults[].assertionResults[]`. This repo runs Vitest 4.
//
// Only the unit suite (`pnpm test`) is reported here. `test:integration` and
// `test:pg` stay in ci.yml, where the Postgres container already exists.
