...
//
// Only three checks run in this repo: the build, the typecheck, and the unit
// tests. There is no linter and no formatter to gate, and the package shape is
// guarded by `pnpm publish --dry-run` in ci.yml rather than by a byte budget.
const POLICY = {
	build: 'no-new',
	typecheck: 'no-new',
	tests: 'no-new',
...
// job passes the gate.
//
// `build` is not here: its fragment is written by the report job from the two
// `build.outcome` files, so it cannot go missing on its own.
