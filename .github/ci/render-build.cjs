...
// The first line of an error in this repo's build output. `pnpm build` is two
// `tsc -p` runs, so an error is either a diagnostic (`src/x.ts(3,5): error
// TS2322: …`) or a bare configuration error (`error TS5083: …`). Both carry
// `error TS<digits>:`. The excerpt is anchored on this rather than on the tail
// of the log, because pnpm prints its own ELIFECYCLE summary after the useful
// part.
const ERROR_HEADING = /(^|\s)error TS\d+:/
